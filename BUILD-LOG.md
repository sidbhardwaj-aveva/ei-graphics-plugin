...
**Files touched:** `plugins/aveva-ei-graphics/skills/ei-graphics-core/schemas/story-understanding.schema.json`,
a new `plugins/aveva-ei-graphics/skills/ei-graphics-core/references/local-input.md`,
`plugins/aveva-ei-graphics/agents/ei-graphics.agent.md`,
`tests/aveva-ei-graphics/skills/ei-graphics-core/schemas/Schemas.Tests.ps1`,
`tests/aveva-ei-graphics/skills/ei-graphics-core/scripts/Write-EiArtifact.Tests.ps1`,
`tests/aveva-ei-graphics/agents/Agent.Tests.ps1`, `tests/PlainLanguage.Tests.ps1` and
`tests/EverythingGreen.Tests.ps1`.

**Acceptance:** The schema tests prove that an understanding is accepted with `inputSource` and no
`adoHash`, that all five kinds are allowed, that an unknown kind is refused, that each of the four
`inputSource` fields is required, and that a payload naming neither source is refused. A writer test
sends a local-input understanding through `Write-EiArtifact.ps1` with no `ado.json` present and gets
exit 0 and a stamped hash, and a second writer test proves the sourceless payload is refused. The
agent tests prove the new route, its link and its four kinds, and five tests cover `local-input.md`
itself. Focused suites 323 passed. Progress check exit 0. Full suite 707 passed, 0 failed, 0 skipped.

**Attempts:** Two. The first focused run failed twice. The agent file carried a 26-word sentence,
one over the limit, and was split in two. Adding `local-input.md` to the scanned-document list made
that list eleven long, so the count assertion in `tests/EverythingGreen.Tests.ps1` and the comment
above it were both raised from ten. The second run was green.

**Decisions:** Express the choice as `anyOf` over `adoHash` and `inputSource`, so a payload with
neither is refused by the schema rather than by prose. Correcting this entry's own assumption: the
plan names four shapes of local input but the schema needs five kinds, because an attached report
and an attached image are separate `kind` values. `local-input.md` was added to the plain-language

...
| `ei-azure-devops-cli-intake` | Resolve a pasted Azure DevOps work item reference to a work item id and retrieve normalized intake content via CLI; reads title, description, acceptance criteria, repro steps and the discussion thread, downloads embedded images from fields and comments, and fixes organization and project to `AVEVA-VSTS` / `Dabacon Products` |

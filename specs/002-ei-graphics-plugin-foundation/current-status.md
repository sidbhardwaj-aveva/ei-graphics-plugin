...
63. Work item reference intake made deterministic and its destination pinned. The real input is always a pasted markdown link, but nothing turned that into a story id — the model did, making the first value of every run non-deterministic. Reference parsing now lives in `ei-azure-devops-cli-intake/scripts/helpers/EiWorkItemReference.ps1` and is shared by `Invoke-EiAdoCliIntake.ps1` and `Start-EiWorkflowRun.ps1`, so `-StoryId` may be left empty when `-StoryRef` carries the link, and `storyRef` is normalised to a bare URL. A markdown link is now found inside surrounding prose instead of being discarded by an anchored regex. Organization and project are fixed — explicit parameters, then `AZDO_ORG`/`AZDO_PROJECT`, then `AVEVA-VSTS` / `Dabacon Products` — and the URL is deliberately not a resolution tier, which both stops a link from moving a run to another project and fixes the short-link defect that sealed `project = "_workitems"` into `ado.json`. Intake failure reasons are documented as a table; `invalid-work-item-id` and `missing-organization-or-project` are gone because the resolver can no longer produce them. Full suite: 307 passed, 0 failed.
...
65. `ado-intake` reference-resolution defect closed. `ei-graphics-workflow/SKILL.md` documents the stage as `Invoke-EiAdoIntakeStage.ps1 -StateDir <dir> -Json`, but the script never read the reference out of `workflow-state.json`, so the documented call always reached `Invoke-EiAdoCliIntake.ps1` with no URL and no id and blocked the run with `EIAI-INTAKE-FAILED (missing-work-item-url-or-id)`. Because `Set-EiWorkflowStage.ps1` has no `unblock` action and refuses `start` on a blocked stage, every retry failed too and the only recovery was `Initialize-EiWorkflowState.ps1 -Force`, which discards the run. The stage now resolves an omitted reference from state — `storyRef` first, then `storyId` — and reports the origin as `Details.ReferenceSource` (`parameter` | `workflow-state.storyRef` | `workflow-state.storyId`). An explicit `-WorkItemUrl`/`-WorkItemId` still wins, so the override path is unchanged. Three focused tests added. The retry-after-block gap is recorded as a known limitation, not fixed in this tranche.
66. Workflow startup consolidated into a single call. Beginning a run cost seven separate script invocations — initialise state, write the session start marker, preflight, then four `Set-EiWorkflowStage.ps1` transitions for `preflight` and `state-init` — and in an agent host every one of those is a separate approval prompt, all paid before the run does any story work. Added `Start-EiWorkflowRun.ps1`, which performs the whole sequence in one process and returns one result contract carrying `StateDir`, `SessionId`, `Resumed`, `StagesCompleted`, `NextStage` and the full `Prerequisites` detail. Consolidation is not permission to skip: each underlying script is still the sole owner of its own step and is still called, so `workflow-state.json` is still mutated only through `Set-EiWorkflowStage.ps1`. It fails closed — a failed `prerequisites` gate records `preflight` as blocked with `EIWF-PREREQUISITES` and returns `EIWF-BOOTSTRAP-PREFLIGHT` rather than leaving a run that looks startable — and a failed session-log write is the one non-fatal step. Re-running is safe: stages already `complete` are left untouched, so `startedAt` is preserved and no gate is re-evaluated. `-StateDir` is deliberately not forwarded to the preflight, because from Phase B that would assert a `candidate.json` which by definition does not exist yet at bootstrap time. `ITERATE`'s second stage is `state-recovery`, which also recovers branch and PR evidence, so it is left `pending` for its owner. This also closes the documentation gap behind entry 65's stage-order block: running a validation script was never the same thing as recording its stage, and the bootstrap now does both. Seven focused tests added; `SKILL.md` Steps 0–3 and `INSTRUCTIONS.md` rewritten around the single call.
